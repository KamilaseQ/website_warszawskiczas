...
export function MobileMenu({ open, onClose }: MobileMenuProps) {
  ...
  const menuContent = (
    <div
      id="mobile-menu"
      className="fixed inset-0 z-[200] flex animate-[wc-mobile-menu-in_280ms_cubic-bezier(0.76,0,0.24,1)_both] flex-col bg-[#0a0a0a] text-white lg:hidden"
    >
      <div
        aria-hidden="true"
            ...
            return (
              <li key={item.href}>
                {item.contactSource ? (
                  ...
                  <Link href={localizePath(item.href, locale)} prefetch={false} onClick={onClose} className={linkClass}>
                    {inner}
                  </Link>
                )}
              </li>
      ...
      <div className="relative border-t border-white/10 px-8 py-6 text-white/60">
        <a
