// Szablony maili formularza. Dwie wiadomości:
//  - buildOwnerEmail   → powiadomienie do butiku (replyTo = klient)
//  - buildAutoReplyEmail → automatyczne potwierdzenie do klienta
// Oba mają wersję HTML i tekstową (multipart/alternative robi nodemailer).
...
const GOLD = '#b89968'
const DARK = '#0a0a0a'
const PHONE = '+48 604 312 411'
const PHONE_TEL = '+48604312411'
const ADDRESS = 'ul. Mokotowska 71, 00-530 Warszawa'
...
<html lang="pl"><body style="margin:0;padding:0;background:#f5f5f4;font-family:-apple-system,Segoe UI,Helvetica,Arial,sans-serif;color:#111;">
  <div style="max-width:600px;margin:0 auto;padding:32px 16px;">
    ...
    <div style="background:#fff;padding:32px 28px;font-size:15px;line-height:1.7;color:#222;">
      <p style="margin:0 0 16px;">Witaj ${escapeHtml(firstName(ctx.name))},</p>
      ...
      <p style="margin:0 0 16px;">W sprawach pilnych jesteśmy do Twojej dyspozycji telefonicznie:</p>
      <p style="margin:0 0 24px;">
        <a href="tel:${PHONE_TEL}" style="display:inline-block;padding:12px 20px;background:${GOLD};color:#fff;text-decoration:none;font-size:13px;letter-spacing:2px;text-transform:uppercase;">${PHONE}</a>
      </p>
      ...
      <p style="margin:24px 0 0;color:#666;font-size:13px;">Z poważaniem,<br/>Zespół Warszawski Czas<br/>${escapeHtml(ADDRESS)}</p>
    </div>
  ...
  const text =
    ...
    `W sprawach pilnych zapraszamy do kontaktu telefonicznego: ${PHONE}\n\n` +
